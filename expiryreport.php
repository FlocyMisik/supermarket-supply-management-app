...
if($month!="" && $year!=""){
    ...
    //extracts and return the value of a specified datetime field 
    $sql="SELECT * FROM GOODS WHERE EXTRACT(MONTH FROM  EXPIRY_DATE)  = '$month' AND  EXTRACT(YEAR FROM EXPIRY_DATE)  = '$year'";
}
    else{
    if($month!=""){
        //if only month is selected output only goods of that month
        $sql="SELECT * FROM GOODS WHERE EXTRACT(MONTH FROM  EXPIRY_DATE)  = '$month'";
    } else if($year!=""){
        ////if only year is selected output only goods of that year
        //extracts and return the value of a specified datetime field 
        $sql="SELECT * FROM GOODS WHERE EXTRACT(YEAR FROM  EXPIRY_DATE)  = '$year'";
    }
    else{
...
<h4 style="text-align:center;">Expiry report</h4>
<table class="tab">
 <tr>
     <th>No:</th>
     <th>Product ID</th>

...
if($month!="" && $year!=""){
    ...
    $sql="SELECT * FROM SALES WHERE EXTRACT(MONTH FROM  SALES_DATE)  = '$month' AND  EXTRACT(YEAR FROM  SALES_DATE)  = '$year'";
}

else{
    if($month!=""){
        //if only month is selected output only sales of that month
        $sql="SELECT * FROM SALES WHERE EXTRACT(MONTH FROM  SALES_DATE)  = '$month'";
    } 
    else if($year!=""){
        ////if only datefrom is selected output only sales upto that selected date
        $sql="SELECT * FROM SALES WHERE EXTRACT(YEAR FROM  SALES_DATE)  = '$year'";
    }
    else{
...
<table class="tab">
 ...
 //associative array is an array where each key has its own specific valuew
